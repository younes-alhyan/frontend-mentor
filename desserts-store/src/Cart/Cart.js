...
export default function Cart({ items, removeFromCart, togglePopup }) {
    let inCart = 0;
    let total = 0;
    items.forEach(item => {
    ...
    return (
        <div className="cart">
            <h2>Your Cart ({inCart})</h2>
            {inCart === 0 ? <Empty />
                : <CartItems items={items} total={total} removeFromCart={removeFromCart} togglePopup={togglePopup} />}
        </div>
    )
}
function Empty() {
    return (<>
        ...
        <p className='empty-cart-text'>You added items will appear here</p>
    </>)
}